...
export default function MatrixView({
  ...
  address: string;
  bundleState: any;
  bundleStateLoading: any;
  bundleId: any;
  bundleStateError: any;
  setBundleId: any;
  requiredTokenStructs: any;
}) {
    ...
    for (let colIndex = 0; colIndex < numberOfColumns; colIndex++) {
      ...
      if (bundleIds && bundleIds.length > 0) {
        ...
        const cellColor = getColor(
          quantity,
          isBurned,
          requiredTokenStructs.map((token: any) => token.totalAmount),
          areMessagesInBundle,
          currBundleId.eq(bundleId)
        );
        rowData.data.push({
          x: `Col ${colIndex + 1}`,
          y: cellValue,
